\ main.fs  JTAG driver for F330 (was: F300).

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ \\\\\\\\\\\\\\\\\\\\    F330 PORT - DIP-20    \\\\\\\\\\\\\\\\\\\\\
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ \\\                                                             \\\
\ \\\   30 Nov Thu 10:37 UTC -- sure looks like a complete port   \\\
\ \\\   to C8051F330, based in AMRFv7.  RESTRICTED to current     \\\
\ \\\   uses of AMRForth v7 -- no F120 support yet.               \\\
\ \\\                                                             \\\
\ \\\   Will test on hardware this week.  cwh                     \\\
\ \\\                                                             \\\
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

in-meta decimal

\ cr .( November 30 2006 2113 UTC port to F330 -- Proven on hardware. ) cr 

  cr
  cr .( November 30 2006 2113 UTC port to F330 ) cr
  cr .(  -- Proven on hardware. ) cr
  cr
...
\ Maximum of 255us allowed, since it uses 8 bits.
...
\ ----- initialization code goes here, before MAIN.
	...
	\ f300: $03 # XBR1 mov  \ Enable TX and RX on P0.4, P0.5.
	\ f330: $01 # XBR0 mov  \ Enable TX and RX on P0.4, P0.5.
	$01 # XBR0 mov  \ Enable TX and RX on P0.4, P0.5.
	\ f300: $40 # XBR2 mov  \ Enable crossbar and weak pull-ups.
	\ f330: $40 # XBR1 mov  \ Enable crossbar and weak pull-ups.
	$40 # XBR1 mov  \ Enable crossbar and weak pull-ups.
\ Setup serial port.
	\ f300: $17 # OSCICN mov  \ Full speed internal, 24.5 MHz.
	\ f330: $c3 # OSCICN mov  \ Full speed internal, 24.5 MHz.
...
code _init  (  - )
	...
	# P0 orl  \ All low.
	\ f300: $40 # XBR2 orl  \ Enable crossbar, weak pullups.
	\ f330: $40 # XBR1 orl  \ Enable crossbar, weak pullups.
	$40 # XBR1 orl  \ Enable crossbar, weak pullups.
	next c;
...
\ \\\\\\\\\\\\\\\ timestamp  Sun 8 Apr 2012  14.19z
...
\ \\\\\\\\\\\\\\\\\\ discrepancy   \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ \\\                                                           \\\
\ \\\   read-byte   is here.   read-flash-byte   is on paper!   \\\
\ \\\                                                           \\\
\ \\\                                                           \\\
\ \\\    response: changed it on paper.                         \\\
\ \\\    the code here stands, as-is.    -cwh 8 Apr 2012.       \\\
\ \\\                                                           \\\
\ \\\    result: snafu.  change it here -- paper knew best.     \\\
\ \\\                                                           \\\
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ \\\\\\\\\\\\\\\   large change just below.  -cwh   \\\\\\\\\\\\\\\\

\  see page 7 of main.fs printout for resolution. -cwh
\  basically, bootloader330.fs contains the definition
\  for 'a: unlock ... ;a' shown below.  Was rescinded
\  from 'main.fs'.
\ 
...
code localC!  ( c addr - )
	...
	A DPH mov  R2 DPL mov  ' drop call       \  \\\\\\\\\\\\\ large change -cwh
	unlock  A @DPTR movx
...
: write-buffer  (  - )
    buffer addr! 64 for  @+ write-flash-byte  next ; \ \\\\\\\\\\\ note this.
...
\ \\\\\\\\\\\\\\\\\\\\\\   NOTE   \\\\\\\\\\\\\\\\\\\\\\\\\\
\   Pay attention to the hardware assignment; in particular,
\   we're going to use the same pin /definitions/ of both
\   MCUs (F300,F330).  So P0.6 on the one is P0.6 on the
\   other, irrespective of the pin /numberings/ on the
\   packages.
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ F330: P0MDOUT programming works the same as F300.
...
\ timestamp:  proofing - Sun 8 Apr 2012 16.22z cwh
...
\ page boundary  -  p.11 ends.   p.12 begins here.
...
\ Run from outside/Tcl, not directly in the interpreter.
: C2-download  (  - )
...
\ page boundary -  p.12 of 13 pages ends.   Begin main.fs p.13 of 13 pages.
...
\ all quiet on the Western Front. -cwh  Sun  8 Apr 2012 16.30z
